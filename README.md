...
> 작성일: 2026-07-17 · 상태: Phase 0 구현 완료(서버 테스트 101개 통과,
> 추출기는 별도 브랜치에서 구현 완료했으나 PR 미생성). 다음은 Phase 1.
...
## 10. 진행 상황 — Phase 0 구현 완료

**한 것** (계획서: `docs/plans/2026-07-17-phase0-deterministic-skeleton.md`):
- *이 레포(`Autoresearch-archmap`)*: 수신 API(`/api/pr-report`, `/api/manifest`)
  + 템플릿 조립(사실 슬롯만) + 저장(파일 기반) + Streamlit [맵]·[PR 피드] 탭
  구현 완료. 테스트 101개 통과.
- *`Autoresearch`(추출기)*: `SKYAHO/Autoresearch` 이슈 #165 / 브랜치
  `feat/165-archmap-extractor`에서 AST 추출기(`architecture.json`/
  `pr-delta.json` 생성) 구현 완료(테스트 381개 통과). **아직 PR 미생성·미머지.**
- 두 레포를 잇는 JSON 스키마(`schemas/*.schema.json`)를 이 레포가 계약으로
  소유.
- **E2E 확인**: 실제 추출기 산출물이 이 서버의 스키마 검증을 통과해 리포트로
  렌더됨을 확인함.

**아직 안 한 것** (CI 워크플로우·PR 코멘트 upsert는 추출기 PR과 함께 진행 예정):
- `Autoresearch` 쪽 CI 워크플로우(PR/머지 트리거)와 서버 POST 연동.
- 추출기 PR 생성·리뷰·머지.
...
**미결로 남은 것**:
- 서버 호스팅·배포 미확정 — 현재는 로컬 실행만(§ 아래 "로컬 실행" 참고).
- `GET /reports/{repo}/{pr}` 접근제어 미확정 — 스펙 §12는 "Phase 0에서 확정"
  이라 했으나 호스팅이 정해지지 않아 Phase 1로 이월.

**성공 기준 진행 상태**: 리뷰어가 코드를 열기 전에 계약·영향을 리포트만으로
파악(달성, GLM 없이도 사실 슬롯만으로 렌더) · 검증됨 주장이 추출기 사실과
1:1(허위 초록 0, `archmap/verify.py` 불변식으로 구조적 보장) · public 표면
바꾼 PR은 sidecar 갱신 없이 머지 불가(Phase 1의 sidecar 게이트로 예정,
미착수) · Phase 0이 LLM 없이도 유용(달성).
...
Phase 0(서버·추출기)은 구현 완료됐고(추출기는 Autoresearch 이슈 #165 /
브랜치 feat/165-archmap-extractor, 아직 PR 미생성), 다음 할 일은
Phase 1(sidecar __arch__ + CI 게이트)이야. 추출기 PR부터 정리할지,
Phase 1 계획부터 쓸지 판단해줘.
```
